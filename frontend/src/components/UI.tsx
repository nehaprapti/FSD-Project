...
export const GlassCard = ({ children, className = '', ...props }: any) => (
  <div className={`glass p-6 ${className}`} {...props}>{children}</div>
);

export const Button = ({ children, variant = 'primary', className = '', ...props }: any) => {
  const base = "px-4 py-3 rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed";
  const variants = {
    primary: "bg-[#FFD600] text-[#0B0B0B] hover:bg-[#E6C200]",
...
export const Input = ({ label, icon: Icon, className = '', ...props }: any) => (
  <div className={`flex flex-col gap-1.5 mb-4 w-full ${className}`}>
    ...
    <div className="relative">
      {Icon && <div className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50"><Icon size={18} /></div>}
      <input className={`w-full bg-black/20 border border-white/10 rounded-xl py-3 text-white focus:border-[#FFD600] focus:outline-none transition-colors ${Icon ? 'pl-11 pr-4' : 'px-4'}`} {...props} />
    </div>
  </div>
);

export const ScreenTransition = ({ children, keyId, className = '' }: { children: React.ReactNode, keyId: string, className?: string }) => (
  <motion.div
    key={keyId}
